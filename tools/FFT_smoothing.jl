...
function FFT_smoothing(time,x,bSmoothing,bPlotting,smoothtype,parameter,filename)
    ...
    #Calculate one-sided spectrum (PSD) -> this is essentially the smoothed one
    ...
    if bSmoothing
        if smoothtype == 1
            ...
            for sectionNo=1:nens
                ...
                section = Int.(section)
                f2, P1, spectrum_smoothed_iter, fdouble, P2  = FFT_smoothing(time[section],x[section],false,false,smoothtype,parameter,filename)
    ...
    if bPlotting==true
        ...
        savefig(spectrum_comparison_fullrange,filename * "_spectrum_comparison_full_range.pdf")
        ...
        savefig(spectrum_comparison,filename * "_spectrum_comparison.pdf")
    end
    
    # Output smoothed spectrum if requested
    if bSmoothing==true
        f = vec(f2)
        spectrum_smoothed = vec(spectrum_smoothed)
        spectrum_unsmoothed = P1
    end

    return f, spectrum_unsmoothed, spectrum_smoothed, fdouble, P2
